...
class Instrument(object):
    ...
    def get_piano_roll(self, fs=100, times=None,
                       pedal_threshold=64, onset=False):
        ...
        pedal_threshold : int
            ...
            Default is 64.
        onset: whether to create onset roll
        ...
        if onset:
            onset_roll = np.zeros((128, int(fs*end_time)))
        ...
        if self.is_drum:
            if onset:
                if times is None:
                    ...
                    return np.zeros((128, times.shape[0])), np.zeros((128, times.shape[0]))
            else:
                if times is None:
                    return piano_roll
                else:
        ...
        for note in self.notes:
            ...
            if onset:
                onset_roll[note.pitch, min(int(note.start*fs), int(fs*end_time)-1)] = 127  # binary indicator
        ...
        for n, (start, end) in enumerate(zip(times[:-1], times[1:])):
            if start < piano_roll.shape[1]:  # if start is >=, leave zeros
                ...
                piano_roll_integrated[:, n] = np.mean(piano_roll[:, start:end],
                                                      axis=1)
        if onset:
            return piano_roll_integrated, onset_roll
        else:
